...
export default function PortalAuthForm({ mode }: { mode: "login" | "signup" }) {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const response = await fetch(
        `/api/customer-auth/${isLogin ? "login" : "signup"}`,
        {
          method: "POST",
          headers: {
  ...
  return (
    <main className="portal-auth-page">
      <Link className="portal-auth-back" href="/">
        ← Back to WaterSource
      </Link>
      <div className="portal-auth-card">
        <div className="portal-auth-brand">
          <span>W</span> WaterSource portal
        </div>
        <p className="eyebrow">
        ...
        <p className="portal-auth-lead">
          {isLogin
            ? "Track deliveries, view your orders, and manage your address."
            : "Keep your WaterSource orders and delivery details in one place."}
        </p>
        ...
        <p className="portal-auth-switch">
          ...
          <Link href={isLogin ? "/portal/signup" : "/portal/login"}>
            {isLogin ? "Create an account" : "Sign in"}
          </Link>
        </p>
        <p className="portal-preview-note">
          Your account is protected with a secure 30-day session.
        </p>
      </div>
    </main>
  );
